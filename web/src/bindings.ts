...
const position = (wf: Workflow, ref: StepRef) => (ref.phase === 'setup' ? ref.index : wf.setup.length + ref.index);
...
export function availableVars(wf: Workflow, ref: StepRef, userColumns: string[] = [], extraVars: string[] = []): VarInfo[] {
  ...
  for (const { step } of stepsBefore(wf, ref)) for (const e of step.extract ?? []) add({ name: e.var, kind: 'step', source: step.name });
...
export function unresolvedVars(wf: Workflow, ref: StepRef, userColumns: string[] = [], extraVars: string[] = []): string[] {
  const step = (ref.phase === 'setup' ? wf.setup : wf.steps)[ref.index];
  if (!step) return [];
...
/** Every variable name already defined by some extractor. */
export function extractorVarNames(wf: Workflow): Set<string> {
  return new Set(orderedSteps(wf).flatMap(({ step }) => (step.extract ?? []).map((e) => e.var)));
}
...
export function stripAuthorization(wf: Workflow, value: string): Workflow {
  ...
  return { ...wf, setup: clean(wf.setup), steps: clean(wf.steps) };
}
